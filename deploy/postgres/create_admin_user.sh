...
read_password() {
  ...
  printf '\n' >&2
  if [[ -z "${value}" ]]; then
    echo "Secure input returned empty in this terminal. Falling back to visible input." >&2
...
if [[ "$APPLY_TO_DB" == "true" ]]; then
  ...
  (
    cd "$REPO_ROOT"
    printf "%s\n" "$SQL" | docker compose -f /opt/barterappbackend/deploy/docker-compose.yml exec -T postgres psql -v ON_ERROR_STOP=1 -U "$DB_USER" -d "$DB_NAME"
  )
