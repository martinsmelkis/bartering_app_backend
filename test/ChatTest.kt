...
class ChatRoutesTest {
        ...
        client.webSocket("/chat") {
            ...
            sendSerialized(
                AuthRequest(
                    userId = senderId,
                    userName = "Sender",
                    peerUserId = offlineRecipientId,
                    publicKey = "pk_sender",
                    timestamp = ts,
                    signature = "${ts}.${senderId}.${offlineRecipientId}"
                )
            )
            ...
            val messageToOffline = ClientChatMessage(
                ...
                data = ChatMessageData(
                    ...
                    senderId = senderId,
                    senderName = "Sender",
                    recipientId = offlineRecipientId,
            ...
            assertTrue(
                errorMessage.error.contains("offline") || errorMessage.error.contains("not found"),
                "Unexpected error message for offline user: ${errorMessage.error}"
            )
    ...
    fun `test sending malformed JSON`() = testApplication {
        ...
        client.webSocket("/chat") {
            ...
            assertTrue(
                errorResponse.error.contains("Invalid message format") ||
                    errorResponse.error.contains("Cannot deserialize"),
                "Unexpected error message for malformed JSON: ${errorResponse.error}"
            )
