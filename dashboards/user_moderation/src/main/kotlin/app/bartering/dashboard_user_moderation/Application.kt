package app.bartering.dashboard_user_moderation

import app.bartering.dashboard_user_moderation.features.network.BackendModerationApiClient
...
private val logger = LoggerFactory.getLogger("UserModerationDashboardApplication")
...
fun Application.module() {
    ...
    val backendApi = BackendModerationApiClient(dashboardConfig, client)
    ...
    install(Sessions) {
        cookie<DashboardSession>("dashboard_user_moderation_session") {
            cookie.path = "/"
...
private fun buildLoginPage(error: String? = null): String = createHTML().run {
    html {
    head {
        title("Barter User Moderation Dashboard Login")
        meta(charset = "UTF-8")
    ...
    body(classes = "bg-slate-100 min-h-screen flex items-center justify-center p-6") {
        div(classes = "w-full max-w-md bg-white shadow-xl rounded-2xl p-8") {
            h1(classes = "text-2xl font-bold text-slate-900 mb-2") { +"User Moderation Dashboard" }
            p(classes = "text-slate-600 mb-6") { +"Sign in to access reported-user and dispute signals." }
...
private fun buildDashboardPage(snapshot: ModerationSnapshot): String = createHTML().run {
    html {
    head {
        title("Barter User Moderation Dashboard")
        meta(charset = "UTF-8")
        meta(name = "viewport", content = "width=device-width, initial-scale=1")
        script(src = "https://cdn.tailwindcss.com") {}
...
private fun buildSummaryFragment(snapshot: ModerationSnapshot): String = createHTML().run {
    div {
    div(classes = "grid gap-4 md:grid-cols-2 mb-4") {
        metricCard(
    ...
    div(classes = "overflow-x-auto rounded-xl border border-slate-200 bg-white") {
        table(classes = "min-w-full text-sm") {
            thead(classes = "bg-slate-50") {
                tr {
                    th(classes = "px-4 py-3 text-left") { +"User ID" }
                    th(classes = "px-4 py-3 text-right") { +"Reports" }
                    th(classes = "px-4 py-3 text-right") { +"Pending Reports" }
                    th(classes = "px-4 py-3 text-right") { +"Actions Taken" }
                    th(classes = "px-4 py-3 text-right") { +"Reviews" }
                    th(classes = "px-4 py-3 text-right") { +"Pending Review Mod" }
                    th(classes = "px-4 py-3 text-right") { +"Disputed" }
                    th(classes = "px-4 py-3 text-right") { +"Scam Flags" }
                }
            }
            tbody {
                if (snapshot.rows.isEmpty()) {
                    tr {
                        td(classes = "px-4 py-4 text-slate-500") {
                            ...
                            +"No moderation rows available yet."
                        }
                    }
                } else {
                    snapshot.rows.forEach { row ->
                        tr(classes = "border-t border-slate-100") {
                            td(classes = "px-4 py-3 font-mono text-xs") { +row.userId }
                            td(classes = "px-4 py-3 text-right") { +row.totalReportsReceived.toString() }
                            td(classes = "px-4 py-3 text-right") { +row.pendingReports.toString() }
                            td(classes = "px-4 py-3 text-right") { +row.actionsTaken.toString() }
                            td(classes = "px-4 py-3 text-right") { +row.totalReviews.toString() }
    ...
    if (!snapshot.connectionError.isNullOrBlank()) {
        div(classes = "mt-4 rounded-lg bg-amber-100 px-4 py-3 text-sm text-amber-800") {
            +"Moderation API warning: ${snapshot.connectionError}"
        }
    }
}
}
...
private fun parseAdminCredentials(raw: String): Map<String, String> {
    ...
    return raw
        ...
        .mapNotNull { entry ->
            ...
            val username = trimmed.take(firstColon).trim()
